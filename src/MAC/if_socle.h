...
//Pre-define
//#define CPU_BYTE_ORDER 	//global.h
//#define DES_BYTE_ORDER	//gloal MAC.h (gMac.h)
//#define DATAB_BYTE_ORDER	//gloal MAC.h (gMac.h)
...
#define MACLONGSWAP(x)	( ( ((ULONG)(x) & (ULONG)0x000000ffUL) << 24) | \
...
#endif /* CPU_BYTE_ORDER == F_BIG_ENDIAN */

#ifndef VERIABLE_BYTE_ORDER
	#if ( CPU_BYTE_ORDER != DES_BYTE_ORDER )
		#define DESSWAP(x)	(MACLONGSWAP(x))
	#else
		#define DESSWAP(x)	(x)
	#endif /* CPU_BYTE_ORDER != DES_BYTE_ORDER */

	#if ( CPU_BYTE_ORDER != DATAB_BYTE_ORDER )
		#define DATABSWAP(x)	(MACLONGSWAP(x))
	#else
		#define DATABSWAP(x)	(x)
	#endif /* CPU_BYTE_ORDER != DATAB_BYTE_ORDER */
#endif
...
#define sMAC_CSR(base,x)			((ULONG)(base) + ((SOCLE_REG_OFFSET) * (x)))
#define sMAC_READ_CSR(base,x)		((ULONG)CSRSWAP(*((ULONG *)sMAC_CSR((base),(x)))))
#define sMAC_WRITE_CSR(base,x,val)	(*((ULONG *)sMAC_CSR((base),(x))) = CSRSWAP((ULONG)(val)))
...
#define CSR0_DBO		((ULONG)0x01 << 20)	//Descriptor byte ordering mode

#define CSR0_TAP_MSK	((ULONG)0x07 << 17)	// Transmit automatic polling mask */
#define CSR0_TAP_VAL(x)	(((ULONG)(x) << 17) & CSR0_TAP_MSK)

#define CSR0_PBL_MSK	((ULONG)0x3F << 8)	// Dma burst length mask
#define CSR0_PBL_VAL(x)	(((ULONG)(x) << 8) & CSR0_PBL_MSK)
...
#define CSR0_BLE		((ULONG)0x01 << 7)	// Big/little endian

#define CSR0_DSL_MSK	((ULONG)0x1F << 2)	// Descriptor skip length
#define CSR0_DSL_VAL(x) (((ULONG)(x) << 2) & CSR0_DSL_MSK)

#define	CSR0_BAR_EQU	0x00000000
#define	CSR0_BAR_RXH	0x00000002
#define CSR0_BAR_VAL(x) (((ULONG)(x) << 1) & CSR0_BAR_RXH)

#define CSR0_SWR		((ULONG)0x01 << 0)	// software reset
...
#define CSR5_TS_MSK		((ULONG)0x07 << 20)
...
#define CSR5_RS_MSK		((ULONG)0x07 << 17)
...
#define CSR5_NIS		((ULONG)0x01 << 16)	// normal interrupt summary
#define CSR5_AIS		((ULONG)0x01 << 15)	// abnormal interrupt summary
#define CSR5_ERI		((ULONG)0x01 << 14)	// early rcv interrupt
#define CSR5_GPTE		((ULONG)0x01 << 11)	// General Purpose Timer Expire
#define CSR5_ETI		((ULONG)0x01 << 10)	// early xmit interrupt
#define CSR5_RPS		((ULONG)0x01 <<  8)	// rcv process stopped
#define CSR5_RU			((ULONG)0x01 <<  7)	// rcv buffer unavailable
#define CSR5_RI			((ULONG)0x01 <<  6)	// rcv interrupt
#define CSR5_UNF		((ULONG)0x01 <<  5)	// xmit underflow
#define CSR5_TU			((ULONG)0x01 <<  2)	// xmit buffer unavailable
#define CSR5_TPS		((ULONG)0x01 <<  1)	// xmit Process stopped
#define CSR5_TI			((ULONG)0x01 <<  0)	// xmit interrupt

/* CSR6 Operation Mode Register */
#define CSR6_SPEED		((ULONG)0x01 << 31)	// speed mode 1:100M
#define CSR6_RA			((ULONG)0x01 << 30)	// Receive all
#define	CSR6_TTM		((ULONG)0x01 << 22)	// transmit thresold mode
#define	CSR6_SF			((ULONG)0x01 << 21)	// store forward mode

#define CSR6_TR_MSK		((ULONG)0x03 << 14)	// Threshold Control Bit
#define CSR6_TR_VAL(x)	(((ULONG)(x) << 14) & CSR6_TR_MSK)

#define	CSR6_ST			((ULONG)0x01 << 13)	// start/stop Xmit command
#define	CSR6_FD			((ULONG)0x01 <<  9)	// Full Duplex mode
#define	CSR6_PM			((ULONG)0x01 <<  7)	// Pass all multicast
#define	CSR6_PR			((ULONG)0x01 <<  6)	// promiscuous mode
#define	CSR6_IF			((ULONG)0x01 <<  4)	// inverse filtering
#define	CSR6_PB			((ULONG)0x01 <<  3)	// pass bad frames
#define	CSR6_HO			((ULONG)0x01 <<  2)	// hash only filtering mode
#define	CSR6_SR			((ULONG)0x01 <<  1)	// start/stop receive command
#define	CSR6_HP			((ULONG)0x01 <<  0)	// hash/perfect recv filtering mode
...
#define	CSR8_OCO		((ULONG)0x01 << 28)	// overflow conter overflow
#define CSR8_FOC_SHF	17					// FIFO overflow counter shift bits
#define CSR8_FOC_MSK	((ULONG)0x7FF << CSR8_FOC_SHF)	// FIFO overflow counter counter mask
#define CSR8_FOC_GET(x)	(((x) & CSR8_FOC_MSK) >> CSR8_FOC_SHF)
#define CSR8_OCOFOC_GET(x)	(((x) & (CSR8_FOC_MSK|CSR8_OCO)) >> CSR8_FOC_SHF)
#define	CSR8_MFO		((ULONG)0x01 << 16)	// missed frame overflow
...
#else	//for MAC with newly LII_A board, arthur
#define      CSR9_MII_RD             0x00000000                      // MII mgmt read mode   //ASIC inverse this
#define      CSR9_MII_WR	     0x00040000		            // MII mgmt write mode  //ASIC inverse this
#endif
...
#define	CSR11_CS			((ULONG)0x01 << 31)	// Cycle size
#define CSR11_CS_VAL(x)		(((ULONG)(x) << 31) & CSR11_CS)

#define CSR11_TT_MSK		((ULONG)0x0F << 27)	// Transmit timer
#define CSR11_TT_VAL(x) 	(((ULONG)(x) << 27) & CSR11_TT_MSK)

#define CSR11_NTP_MSK		((ULONG)0x07 << 24)	// Number of transmit packet
#define CSR11_NTP_VAL(x)	(((ULONG)(x) << 24) & CSR11_NTP_MSK)
#define CSR11_NTP_GET(x)	(((x) & CSR11_NTP_MSK) >> 24)

#define CSR11_RT_MSK		((ULONG)0x0F << 20)	// Receive timer
#define CSR11_RT_VAL(x) 	(((ULONG)(x) << 20) & CSR11_RT_MSK)

#define CSR11_NRP_MSK		((ULONG)0x07 << 17)	// Transmit timer
#define CSR11_NRP_VAL(x)	(((ULONG)(x) << 17) & CSR11_NRP_MSK)
#define CSR11_NRP_GET(x)	(((x) & CSR11_NRP_MSK) >> 17)

#define	CSR11_CON_MODE	0x00010000			// GPT Continuous Mode
#define CSR11_CON_VAL(x) 	(((ULONG)(x) << 16) & CSR11_CON_MODE)
#define	CSR11_TIM_MASK	0x0000FFFF			// GPT Timer Value Mask
#define CSR11_TIM_VAL(x) 	(((ULONG)(x)      ) & CSR11_TIM_MASK)
...
#define FLTR_HASH_FRM_SIZE_ULONGS	(FLTR_HASH_FRM_SIZE / sizeof (ULONG))
