...
int	sMacInit(	DRV_CTRL *	pDrvCtrl	)
{
  ...
  SYS_INT_CONNECT(pDrvCtrl, sMacInt, pDrvCtrl, &result);
	//SYS_INT_SET_TYPE(pDrvCtrl, HI_LEVEL);	//leonid mark for re set  irq type 20090820
  ...
  //SYS_INT_ENABLE (pDrvCtrl);		//leonid mark for re enable irq 20090820
    ...
    )
{
	...
	for (i = 0; i < tblLen; i++)
	{
		index = ether_crc_le(FLTR_FRM_ADRS_SIZE, (unsigned char *)pPhysAdrsTbl) & 0x1ff;
