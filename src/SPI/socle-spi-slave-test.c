...
socle_spi_slave_run(int type)
{
  ...
  socle_spi_write(
	...
	if(type==SLAVE_RESET) {
		...
		memset((char *)socle_spi_pattern_buf, 0,PATTERN_BUF_SIZE );
		memset((char *)socle_spi_cmpr_buf, 0, PATTERN_BUF_SIZE);
		/* Make test pattern */
...
socle_spi_slave_pure_run(void)
{
  ...
  socle_spi_write(
	...
	/* Clear pattern buffer and compare buffer */
	memset((char *)socle_spi_pattern_buf, 0, PATTERN_BUF_SIZE_PURE*2);
	memset((char *)socle_spi_cmpr_buf, 0, PATTERN_BUF_SIZE_PURE*2);
	/* Make test pattern */
	...
	if(socle_spi_get_current_mode() == MODE_CHAR_LEN_8){
		...
		memcpy((char *)&pdata8[1], (char *)socle_spi_pattern_buf, PATTERN_BUF_SIZE_PURE);
  	...
  	printf("compare data...\n");
		...
		memcpy((char *)&pdata16[1], (char *)socle_spi_pattern_buf, PATTERN_BUF_SIZE_PURE*2);
...
socle_spi_slave_write_data(u16 addr, u8 *buf, u16 len, int type)
{
	...
	while(i<len) {
		...
		if( stat & SOCLE_SPI_SLAVE_RxFF_EMPTY ) {
			memcpy((char *)&tx_buf[1], (char *)&buf[i], SOCLE_SPI_FIFO_SIZE);	
			...
			memcpy((char *)&tx_buf[1], (char *)&buf[i], SOCLE_SPI_FIFO_SIZE/2);	
