...
socle_spi_marvel_wifi(int autotest)
{
	...
	memset((char *)socle_spi_pattern_buf, 0x0, MARVEL_MAX_NUM * MARVEL_CHAR_LEN / 8);
	memset((char *)socle_spi_cmpr_buf, 0x0, MARVEL_MAX_NUM * MARVEL_CHAR_LEN / 8);
	...
	for (i = 0; i < 10; i++) {
		...
		memcpy((char *)&tx_buf[1], (char *)socle_spi_pattern_buf, MARVEL_MAX_NUM * MARVEL_CHAR_LEN / 8);
